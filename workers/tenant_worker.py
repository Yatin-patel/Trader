...
class TenantWorker:
    ...
    async def _run_one_cycle(self) -> int:
        ...
        # NB: pre-market / after-hours strategy graph IS allowed.
        # Alpaca queues options orders submitted outside RTH so they
        # fill at the open. Previously we gated the strategy graph out
        # of pre-market to prevent a flood of duplicate queued orders.
        # The order_guard module (risk/order_guard.py) now suppresses
        # duplicate-symbol submissions across cycles, so the original
        # 1,650+-duplicates failure mode is impossible. Trading the
        # 04:00-09:30 ET window captures momentum-on-open opportunities
        # we were losing — the user explicitly asked for this.
